...
class DistributionsManager:
    ...
    def createBinOperation(self, leftoperand, operator, rightoperand, poly_precision, regularize=True,
                           convolution=True):
        name = "(" + leftoperand.name + str(operator) + rightoperand.name + ")"
        if name in self.distrdictionary:
            return self.distrdictionary[name]
        else:
            tmp = BinOpDist(leftoperand, operator, rightoperand, poly_precision, self.samples_dep_op, regularize,
                            convolution)
...
class TreeModel:
    ...
    def evaluate(self, tree):
        ...
        # quantized_distribution will be the quantized operation on the quantized children nodes
            ...
            dist = self.manager.createBinOperation(tree.left.root_value[0], tree.root_name, tree.right.root_value[0],
                                                   self.poly_precision, convolution=tree.convolution)
    ...
    def collectInfoAboutDistribution(self, f, finalDistr_wrapper, name, golden_mode, bin_val):
        ...
        for i in [0.25, 0.5, 0.75, 0.85, 0.95, 0.99, 0.9999]:
            ...
            while val < i:
                ...
                if val >= i:
                    res = res + "Range: [" + str(lower) + "," + str(upper) + "] contains " + str(
                        i * 100) + "% of the distribution.\n\n"
                    break
        res = res + "Range: [" + str(finalDistr_wrapper.a) + "," + str(
            finalDistr_wrapper.b) + "] contains 100% of the distribution.\n\n"
        res = res + "###########################################\n\n"
        # print(res)
        f.write(res)
        return
    ...
    def collectInfoAboutSampling(self, f, vals, edges, name, golden_mode=None, golden_ind=None):
        ...
        for i in [0.25, 0.5, 0.75, 0.85, 0.95, 0.99, 0.9999]:
            ...
            res = res + "Range: [" + str(edges[lower]) + "," + str(edges[upper]) + "] contains " + str(
                i * 100) + "% of the distribution.\n\n"
    ...
    def measureDistances(self, fileHook, vals_PM, vals_golden, vals, edges_PM, edges_golden, edges, introStr,
        ...
        fileHook.write(
            "AVG Variational Distance - Golden -> Sampling : " + str(avg_var_distance_golden_sampling) + "\n")
        ...
        for fp_or_real in [False]:
            ...
            for binLen in [50, 100, 500]:  # , 1000, 5000, 10000, 15000]:
                ...
                if fp_or_real:
                    ...
                    fileHook.write("Picked Precision for histogram. mantissa:" + str(tmp_precision) + ", exp:" + str(
                        tmp_exp) + "\n\n")
                    fileHook.flush()
                ...
                vals_golden, edges_golden, patches_golden = plt.hist(golden_samples, bins, density=True, color="black",
                                                                     label="Golden model")
                golden_mode, golden_ind = self.collectInfoAboutSampling(golden_file, vals_golden, edges_golden,
                                                                        "Golden with num. bins: " + str(binLen))
                self.outputEdgesVals(golden_file,
                                     "BinLen: " + str(binLen) + ", FP_or_real: " + str(fp_or_real) + "\n\n",
                                     edges_golden, vals_golden)
                golden_file.close()

                self.collectInfoAboutDistribution(fileHook, self.tree.root_value[2],
                                                  "Range Analysis on Round(distr) with " + str(binLen) + " bins",
                                                  golden_mode, binLen)

                pm_file = open(path + file_name + "/pm.txt", "a+")
                vals_PM, edges_PM, patches_PM = plt.hist(self.tree.root_value[2].distributionValues, edges_golden,
                                                         density=True, alpha=0.0, color="red")
                self.collectInfoAboutSampling(pm_file, vals_PM, edges_PM, "PM with num. bins: " + str(binLen),
                                              golden_mode=golden_mode, golden_ind=golden_ind)
                self.outputEdgesVals(pm_file, "BinLen: " + str(binLen) + ", FP_or_real: " + str(fp_or_real) + "\n\n",
                                     edges_PM, vals_PM)

                sampling_file = open(path + file_name + "/sampling.txt", "a+")
                vals, edges, patches = plt.hist(r, edges_golden, alpha=0.5, density=True, color="blue",
                                                label="Sampling model")
                self.collectInfoAboutSampling(sampling_file, vals, edges, "Sampling with num. bins: " + str(binLen),
                                              golden_mode=golden_mode, golden_ind=golden_ind)
                self.outputEdgesVals(sampling_file,
                                     "BinLen: " + str(binLen) + ", FP_or_real: " + str(fp_or_real) + "\n\n", edges,
                                     vals)
                sampling_file.close()

                self.measureDistances(fileHook, vals_PM, vals_golden, vals, edges_PM, edges_golden, edges,
                                      "Range Analysis comparison. Bins: " + str(
                                          binLen) + ", Floating Point Spacing: " + str(fp_or_real), fp_or_real)
    ...
    def plot_empirical_error_distribution(self, summary_file, finalTime, benchmarks_path, file_name, abs_fpt, rel_fpt):
        ...
        abs_err = UnOpDist(
            BinOpDist(self.tree.root_value[2], "-", self.tree.root_value[0], 500, regularize=True, convolution=False),
            "abs_err", "abs")
