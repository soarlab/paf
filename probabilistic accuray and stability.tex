...
\begin{align*}
\Pro{\mathtt{Prog}(X,Y)=\mathtt{FALSE}\mid X+Y> C}&=\Pro{X+\fp Y\in\{(-\infty,C]\}\mid X+Y>C}\\
&=\Pro{u(X+Y)\in\{(-\infty,C]\}\mid X+Y>C}\\
&=\int_C^\infty u(z)((-\infty,C]) f_+(z)~dz\\
&=\int_C^{\ceil{C}} u(z)((-\infty,C]) f_+(z)~dz\\
&=\int_C^{\ceil{C}} \frac{\ceil{C}-C}{\ceil{C}-\floor{C}} f_+(z)~dz
\end{align*}
