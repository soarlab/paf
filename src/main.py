...
def process_file(xs, file, mantissa, exp, range_my_dict, abs_my_dict):
    try:
        ...
        plot_error_analysis_CDF(T.abs_err_distr, loadedGolden, abs_err_samples,
                                abs_err_golden, f, file_name, abs_my_dict.get(file_name), rel_my_dict.get(file_name),
                                absORrel="Abs")

        plot_error_analysis_CDF(T.relative_err_distr, loadedGolden, rel_err_samples,
                                rel_err_golden, f, file_name, abs_my_dict.get(file_name), rel_my_dict.get(file_name),
                                absORrel="Rel")
        ...
        plot_range_analysis_PDF(T.final_quantized_distr, loadedGolden, values_samples, values_golden, f, file_name,
                                range_my_dict.get(file_name))
        plot_error_analysis_PDF(T.abs_err_distr, loadedGolden, abs_err_samples, abs_err_golden, f, file_name,
                                abs_my_dict.get(file_name), rel_my_dict.get(file_name))
...
warnings.warn(
    "Mantissa with implicit bit of sign. In gmpy2 set precision=p includes also sign bit. (e.g. Float32 is mantissa=24 and exp=8)\n")
