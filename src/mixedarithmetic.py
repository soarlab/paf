...
from plotting import plot_operation, plotCDFdiscretization, plot_boxing
from project_utils import round_number_down_to_digits, round_number_up_to_digits, dec2Str, round_down, round_up, \
    round_near
from setup_utils import gap_cdf_regularizer, digits_for_discretization, digits_for_cdf, delta_error_computation
...
def from_PDFS_PBox_to_DSI(insiders, evaluation_points):
    ...
    res_ub = Decimal("0")#Interval("0.0", "0.0", True, True, digits_for_cdf)
    acc_ub = Decimal("0")#Interval("0.0", "0.0", True, True, digits_for_cdf)
    res_lb = Decimal("0")#Interval("0.0", "0.0", True, True, digits_for_cdf)
    acc_lb = Decimal("0")#Interval("0.0", "0.0", True, True, digits_for_cdf)
...
def adjust_values_sum_one(values_lower, values_upper):
    for index_low in range(1,len(values_lower)):
        low_value=values_lower[index_low]
        if values_upper[index_low-1] < low_value:
            values_upper[index_low - 1] = low_value
    return values_lower, values_upper

def adjust_max_and_min(values_lower, values_upper):
    #values_lower[0]=Decimal("0.0")
    #values_upper[0]=Decimal("0.0")
    values_lower[-1] = Decimal("1.0")
    values_upper[-1] = Decimal("1.0")
    return values_lower, values_upper
...
def from_DSI_to_PBox(edges_lower, values_lower, edges_upper, values_upper):
    ...
    #values_lower,values_upper=adjust_values_sum_one(values_lower,values_upper)
    #values_lower,values_upper=adjust_max_and_min(values_lower, values_upper)
    ...
    for pair_value_upper in pair_values_upper:
        #the last element of the upper bound does not connect!
        #if pair_value_upper[2]==len(edges_lower)-1:
        #    continue
        #we use bisect left because in case of multiple values we pick the first of them
    ...
    #plotCDFdiscretization(ret_list)
    return ret_list